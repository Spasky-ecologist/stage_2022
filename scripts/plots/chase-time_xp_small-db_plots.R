...
tab <- readRDS(file.path(path, "CT_xp_table_small-db.rds"))
...
plot1 <- ggplot(tab[Parameter == "mu"],
  ...
  scale_y_continuous(breaks = seq(168, 192, 4),
                     limits = c(168, 192)) +
...
plot2 <- ggplot(tab[Parameter == "sigma"],
  ...
  scale_y_continuous(breaks = seq(5.5, 8, 0.5),
                     limits = c(5.5, 8)) +
  scale_x_discrete(expand = c(0,1)) +
...
# Save figure
ggexport(figure,
         filename = file.path(path, "CT_xp_small-db.png"),
         width = 3500,
         height = 1600,
         res = 300)
...
# Save figure for v2
ggexport(figure,
         filename = file.path(path, "CT_xp_small-db_v2.png"),
         width = 3500,
         height = 1600,
         res = 300)
